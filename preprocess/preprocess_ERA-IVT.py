...
### SAVE EACH SET OF FILES AS SINGLE IVT FILE    

for i, community in enumerate(lbl1):
    df_lst = []
    for j, yr in enumerate(range(start_yr, end_yr+1)):
        df = pd.read_csv(path_to_work + '{0}_IVT_{1}.csv'.format(community, yr))
        df_lst.append(df)
